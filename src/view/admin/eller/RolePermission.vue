<template>
  <div class="RolePermission-Container" v-loading="loading">
...
const getGroupPermissions = async () => {
  allPermissions.value = (await AdminModel.getAllPermissions()).rows

  // 还要处理一下
  const perTree = {}

  for ( let per of allPermissions.value ) {

    const array = perTree[per.module] || []

    array.push(per)

    perTree[per.module] = array

  }

  allPermissions.value = perTree
