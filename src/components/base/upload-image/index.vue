...
<template>
<!--  v-loading="loading"-->
  <div class="UploadImgs-Container" :class="{ round }">
    <div v-for="(item, i) in itemList" :key="item.id">
      <div v-if="item.display">
<!--        v-loading="item.loading"-->
        <div class="thumb-item" :style="boxStyle">
          <el-image
            :fit="fit"
            :ref="setImageRef"
            :src="item.display"
...
<!--          v-if="showControlBtn"-->
          <div class="control">
            <div v-if="showControlBtn && !disabled" class="del" @click.prevent.stop="delItem(item.id)" title="删除">
              <el-icon>
                <Close />
              </el-icon>
            </div>
            ...
            <div class="control-bottom" v-if="showControlBtn && (sortable || preview)">
              <i
...
<script>
import { post } from '~/plugins/api/axios'
...
export default {
  ...
  computed: {
    ...
    rulesTip() {
      ...
      // 文件大小
      if (this.showFileSize && (basicRule.minSize || basicRule.maxSize)) {
  ...
  methods: {
    ...
    uploadCatch(uploadList) {
      ...
      // console.log(formData.getAll("file"), uploadList)

      return post('/file/upload', data)
          .then(res => {
            const paths = res.paths

            // console.log(Array.isArray(paths), paths.length)

            if (!Array.isArray(paths) || paths.length === 0) {
              throw new Error('图像上传失败')
            }

            // const resObj = paths.reduce((acc, item) => {
            //   acc[item.key] = item
            //   return acc
            // }, {})

            uploadList.forEach((item, index) => {
              const remoteData = paths[index]//resObj[`file_${index}`]
              item.cb(remoteData)
            })

            this.$emit('upload', paths)
          })
          .catch(err => {
  ...
  components: { Close, Edit }
}
</script>

<style lang="scss" scoped>
.UploadImgs-Container.round {
  border-radius: 50%;
  .upload-item,
  .thumb-item {
    border-radius: 50%;
...
.UploadImgs-Container {
  ...
  .thumb-item {
    ...
    //width: calc(var(--width, 100%) - 2px);
    //height: calc(var(--height, 100%) - 2px);
